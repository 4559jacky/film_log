...
public class EmailVerificationServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String subject = "FilmLog 회원가입 인증 코드";
        String body = "환영합니다. 회원가입을 위한 인증코드는 '" + verificationCode + "'입니다.";
        ...
        EmailCode ec = new MemberService().checkEmailCode(param);
        ...
        if(member == null) {
        	if(ec != null) {
                ...
                result2 = new MemberService().updateEmailCode(paramMap);
                ...
                paramMap.put("email", email);
                paramMap.put("code",verificationCode);
                result2 = new MemberService().insertEmailCode(paramMap);
        	}
        	
        	// 이메일 발송
            if(result2 > 0) {
            	obj.put("res_code", "200");
            	obj.put("res_msg", "이메일로 인증번호를 보냈습니다.");
            } else {
            	obj.put("res_code", "500");
            	obj.put("res_msg", "이메일 전송에 실패했습니다.");
            }
        } else {
        	obj.put("res_code", "500");
        	obj.put("res_msg", "이미 사용중인 이메일입니다.");
        }
