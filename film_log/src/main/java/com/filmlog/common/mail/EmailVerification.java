 ...
 * java.util.Random;
 * 
 * public class EmailVerification { public static void main(String[] args)
 * throws FileNotFoundException, IOException { String email =
 * "user-email@example.com"; // 사용자 이메일 입력 받기 String verificationCode =
 * generateVerificationCode(); String subject = "회원가입 이메일 인증번호"; String body =
 * "회원가입을 위한 인증번호는 " + verificationCode + "입니다.";
 * 
 * // 이메일 발송 EmailSender.sendEmail(email, subject, body);
 * 
